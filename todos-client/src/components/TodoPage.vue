<template>
  <div class="container">
    <h2 style="text-align: left;">Todo List</h2>
    ...
    <ul class="list-group">
      <li class="list-group-item" v-for="(todo,index) in todos" :key="todo.name">
        {{todo.name}}
  ...
  methods: {
    deleteTodo(i) {
      ...
      this.todos.forEach(function(_todo, i, obj) {
        if (_todo.id === todo.id) {
          vm.$http
            .delete("https://todos.garam.xyz/api/todos/" + todo.id)
            .then(result => {
              obj.splice(i, 1);
            });
        }
    ...
    createTodo(name) {
      if (name != null) {
        ...
        this.$http
          .post("https://todos.garam.xyz/api/todos", { name: name })
          .then(result => vm.todos.push(result.data));
